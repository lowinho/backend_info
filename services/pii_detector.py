...
#     def get_last_invalid_cpf_count(self) -> int:
#         """Retorna a contagem de CPFs inválidos da última execução e reseta o contador."""
#         count = self.last_invalid_cpf_count
#         self.last_invalid_cpf_count = 0 # Reseta para a próxima linha/chamada
#         return count
import pandas as pd
import spacy
import re
...
class PIIDetector:
    """
    Detector de PII V10.0 (Sincronizado com Terminal)
    - Validação Contextual de CPF
    - Novas categorias: Registros Gerais, Raça, Gênero
    - Compatível com ReportService V10
    """
    ...
    # Mesmas listas do Terminal para garantir consistência
    ...
    def __init__(self):
        try:
            # Tenta carregar Spacy otimizado
            print("⏳ Tentando carregar modelo Spacy na API...") # DEBUG
            self.nlp = spacy.load("pt_core_news_lg")
            pipes_to_disable = ['parser', 'tagger', 'morphologizer', 'lemmatizer']
            existing_pipes = [p for p in pipes_to_disable if p in self.nlp.pipe_names]
            if existing_pipes:
                self.nlp.disable_pipes(existing_pipes)
            print("✅ Modelo Spacy carregado com SUCESSO na API!") # DEBUG
        except OSError:
            print("❌ ERRO: Modelo Spacy não encontrado na API. Usando apenas Regex.") # DEBUG
            self.nlp = None
    ...
    def _validate_cpf_digit(self, cpf: str) -> bool:
        """Validação matemática de CPF (Idêntica ao terminal)"""
    ...
    def _has_cpf_context(self, text: str, position: int, window: int = 50) -> bool:
        """Verifica contexto ao redor de 11 dígitos soltos"""
    ...
    def _detect_cpf(self, text: str) -> List[Tuple[int, int, bool]]:
        """Lógica V10: Só aceita 11 dígitos soltos se tiver contexto ou formatação"""
        cpf_matches = []
        detected_positions = set()
        
        # 1. Formatado (sempre pega)
        ...
        for match in re.finditer(loose_pattern, text):
            ...
            # AQUI ESTÁ A DIFERENÇA: Só aceita se tiver contexto
            if self._has_cpf_context(text, match.start()):
                is_valid = self._validate_cpf_digit(match.group())
                cpf_matches.append((match.start(), match.end(), is_valid))
                detected_positions.update(range(match.start(), match.end()))
        
        return cpf_matches

    def detect_and_redact(self, text: str) -> Tuple[str, Dict[str, int], int]:
        """
        Retorna:
        1. Texto mascarado
        2. Estatísticas de PII
        3. Contagem de CPFs inválidos (int) para o ReportService
        """
        if pd.isna(text) or not isinstance(text, str):
            return text, {}, 0
        ...
        indices_to_mask = set()
        pii_stats = defaultdict(int)
        invalid_cpf_count = 0 # Contador local para esta string
            ...
            try:
                ...
                for ent in doc.ents:
                    if ent.label_ == "PER":
                        ...
                        if not has_common_part and not has_honorific: continue
        ...
        return "".join(redacted_chars), dict(pii_stats), invalid_cpf_count
